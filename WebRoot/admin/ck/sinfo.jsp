...
<script  language="javascript" >
    function top2(){
        form3.action="<%=basePath%>admin/ck/sinfo.jsp?page=1";
        form3.submit();
    }
    function last2(){
        if(form3.pageCount.value==0){//如果总页数为0，那么最后一页为1，也就是第一页，而不是第0页
            ...
            form3.action="<%=basePath%>admin/ck/sinfo.jsp?page="+form3.pageCount.value;
            form3.submit();
        }
    }
    function pre2(){
        ...
        if(page<=1){
            ...
            form3.action="<%=basePath%>admin/ck/sinfo.jsp?page="+(page-1);
            form3.submit();
        }
    }
    ...
    function bjump2(){
        ...
        if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
            if(pageCount==0){
                ...
                form3.action="<%=basePath%>admin/ck/sinfo.jsp?page="+pageCount;
                form3.submit();
            }
        }
            ...
            if(page==0){
                ...
                form3.action="<%=basePath%>admin/ck/sinfo.jsp?page="+page;
                form3.submit();
            }else{
                form3.action="<%=basePath%>admin/ck/sinfo.jsp?page="+page;
                form3.submit();
...
<div class="page-content clearfix">
    <div class="sort_style">
        <form name="sform" method="post" action="<%=basePath%>admin/ck/sinfo.jsp">
       ...
       <span class="l_f">
        <a href="<%=basePath%>admin/ck/add.jsp" class="btn btn-warning"><i class="fa fa-plus"></i> 添加</a>
        <a onclick="checkchoose()" class="btn btn-danger"><i class="fa fa-trash"></i> 批量删除</a>
       </span>
        </div>
        <div class="sort_list">
            <form action="<%=basePath%>ck/batchDel" method="post" name="form1">
            ...
            <%
                ...
                int cou = cb.getMessageCount("select count(*) from ck where ckbh like '%"+ckbh+"%' and mc like '%"+mc+"%' ");//得到信息总数
                ...
                List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from ck where ckbh like '%"+ckbh+"%' and mc like '%"+mc+"%' order by id desc",11);
            ...
            <tr>
                ...
                <td class="td-manage">
                    <a title="删除" onclick="if (confirm('确定要删除吗？')) return true; else return false;" href="<%=basePath%>ck/del/<%=pagelist2.get(0).toString()%>" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
                </td>
            </tr>
            <%}}%>
        </table>
