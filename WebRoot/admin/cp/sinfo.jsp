...
<div class="page-content clearfix">
    <div class="sort_style">
        <form name="" method="post" action="<%=basePath%>admin/cp/sinfo.jsp">
       ...
       <span class="l_f">
        <a href="<%=basePath%>admin/cp/add.jsp?method=addcp" class="btn btn-warning"><i class="fa fa-plus"></i> 添加</a>
        <a onclick="checkchoose()" class="btn btn-danger"><i class="fa fa-trash"></i> 批量删除</a>
       </span>
        </div>
        <div class="sort_list">
            <form action="<%=basePath%>cp/batchDel" method="post" name="form1">
            ...
            <%
                ...
                int cou = cb.getMessageCount("select count(*) from cp where mc like '%"+mc+"%' ");//得到信息总数
                ...
                List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from cp where mc like '%"+mc+"%' order by id desc",4);
            ...
            <tr>
                ...
                <td class="td-manage">
                    <a title="编辑" href="<%=basePath%>admin/cp/add.jsp?method=upcp&id=<%=pagelist2.get(0).toString()%>"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>
                    <a title="删除" onclick="if (confirm('确定要删除吗？')) return true; else return false;" href="<%=basePath%>cp/del/<%=pagelist2.get(0).toString()%>" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
                </td>
            </tr>
            <%}}%>
        </table>
