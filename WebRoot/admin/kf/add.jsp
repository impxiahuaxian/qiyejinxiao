...
<%
    ...
    if(username==null){
        ...
        String id="";String mc="";String bz="";
        if(method.equals("upkf")){
            id=request.getParameter("id");
            List alist=cb.get1Com("select * from kf where id='"+id+"'",3);
            mc=alist.get(1).toString();
            bz=alist.get(2).toString();
        }
%>
<body>
<div class="sort_style_add margin" id="sort_style_add" style="display:yes">
    <form method="post" action="<%=basePath%>kf/addMethod">
        <input type="hidden" name="method" value="<%=method%>" /><input type="hidden" name="id" value="<%=id%>"/>
        <div class="">
            <ul>
                <li><label class="label_name">客户名称：</label>
                    <div class="col-sm-9">
